...
function PaymentDialog(props: {
  ...
  if (!props.showPaymentModal) return null;
  return (
    <div className="fixed top-0 w-full h-screen bg-black/50 bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl max-w-md w-full p-6">
        <h3 className="text-xl font-semibold">Make Payment</h3>
        <p className="mb-3 text-sm">Select one of the payment methods to proceed</p>
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <button className="relative w-full py-3 bg-[#2587c8] text-white rounded-lg cursor-pointer hover:-translate-y-0.5 transition flex items-center justify-center gap-5">
              ...
              <span className="absolute right-5"><img src={visa} alt="" /></span>
            </button>
            <button className="relative w-full py-3 bg-green-500 text-white rounded-lg hover:-translate-y-0.5 cursor-pointer transition flex items-center justify-center gap-5">
              <i className="fi fi-rr-indian-rupee-sign flex items-center justify-center"></i>
              <span>Pay with UPI</span>
              <span className="absolute right-5"><img src={upi} alt="" className="w-8"/></span>
            </button>
          </div>
          <button
